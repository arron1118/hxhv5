...
<?php
$this_list = ACTION_NAME=='my_jlk_list' ? U('Jzjlk/my_jlk_list') : U('Jzjlk/jlk_list'); ?>
	...
	<blockquote class="layui-elem-quote news_search">
		<form class="layui-form" id="form" method="post" action="<?php echo ($this_list); ?>">
			...
			<div class="layui-inline">
				<a href="<?php echo U('Users/publish',array('type'=>1));?>" class="layui-btn layui-btn-normal">发布信息</a>
			</div>
			<?php if(authOne('Admin/Jzjlk/delete') == true): ?><div class="layui-inline">
	  	...
	  	<table class="layui-table">
			...
			<?php if(is_array($data)): foreach($data as $key=>$v): $one_user = M('admin_users')->field('person')->where(array('id'=>$v['user_id']))->find(); ?>
				<tr>
					...
					<td><a href="/resume/<?php echo idEncode($v['id']);?>.html" target="_blank"><?php echo ($v['person']); ?></a></td>
					<th><?php echo C('ZZZT_TYPE')[$v['cerstatus']];?></th>
				    <td><?php echo C('PRICE_TYPE')[$v['price']];?></td>
				    <td><?php echo C('REGTYPE')[$v['regtype']];?></td>
					...
					<td>
						<input type="checkbox" name="examine" listId="<?php echo ($v['id']); ?>" lay-skin="switch" lay-text="是|否" lay-filter="isShow" <?php if(($v['examine']) == "1"): ?>checked=""<?php endif; ?>>
						<div class="layui-unselect layui-form-switch" lay-skin="_switch"><em><?php if(($v['examine']) == "1"): ?>是<?php else: ?>否<?php endif; ?></em><i></i>
						</div>
					</td>
					...
					<?php if(authOne('Admin/Jzjlk/delete') == true): ?><a class="layui-btn layui-btn-danger layui-btn-mini delete" listId="<?php echo ($v['id']); ?>"><i class="layui-icon">&#xe640;</i> 删除</a><?php endif; ?>
					</td>
	...
	<script type="text/javascript">
		layui.config({
				...
				$ = layui.jquery;
					$("body").on("click",".preview",function(){  //预览
						...
						var index = layui.layer.open({
							...
							title : "预览信息",
							type : 2,
							content : "<?php echo U('Jzjlk/preview');?>?id="+listId,
			...
			form.on('switch(isShow)', function(data){
				...
				setTimeout(function(){
					$.post("<?php echo U('Jzjlk/edit_jlk_status');?>",{id:listId,status:status},function(data){
						top.layer.msg(data);
					})
					layer.close(index);
				},500);
			...
			$("body").on("click",".delete",function(){
				...
				layer.confirm('确定删除此信息？',{icon:3, title:'提示信息'},function(data){
						...
						setTimeout(function(){
							$.post("<?php echo U('Jzjlk/delete');?>",{id:listId},function(data){
			...
			$(".batchDel").click(function(){
				var $checkbox = $('.jlk_list tbody input[type="checkbox"][name="id[]"]');
				var $checked = $('.jlk_list tbody input[type="checkbox"][name="id[]"]:checked');
				if($checkbox.is(":checked")){
					// 异步提交多选数组
					var _list = {};
					$('.jlk_list tbody input[type="checkbox"][name="id[]"]:checked').each(
					function (i) {
					...
					layer.confirm('确定删除选中的信息？',{icon:3, title:'提示信息'},function(index){
						...
						setTimeout(function(){
							$.post("<?php echo U('Jzjlk/delete');?>",{id:_list},function(data){
								top.layer.close(index);
								top.layer.msg(data);
								//刷新当前页面
								window.location.reload();
