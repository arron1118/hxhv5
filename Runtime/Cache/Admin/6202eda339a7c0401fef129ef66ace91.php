	...
	<form class="layui-form" method="post">
		...
		<div class="layui-form-item">
			<div class="layui-inline">
				...
				<?php if(is_array($data)): foreach($data as $key=>$v): ?><input class="tuijian" type="checkbox" name="group_ids[]" value="<?php echo ($v['id']); ?>" title="<?php echo ($v['title']); ?>" <?php if(in_array(($v['id']), is_array($group_data)?$group_data:explode(',',$group_data))): ?>checked="checked"<?php endif; ?>><?php endforeach; endif; ?>
				</div>
			</div>
		</div>
		<div class="layui-form-item">
		...
		<div class="layui-form-item">
			...
			<div class="layui-input-block">
				<input type="radio" name="status" value="1"  title="允许登录" <?php if(($user_data['status']) == "1"): ?>checked="checked"<?php endif; ?>>
				<input type="radio" name="status" value="0"  title="禁止登录" <?php if(($user_data['status']) == "0"): ?>checked="checked"<?php endif; ?>>
				<input type="radio" name="status" value="2" title="未验证" <?php if(($user_data['status']) == "2"): ?>checked="checked"<?php endif; ?>>
			</div>
		</div>
		<hr/>
            <div class="layui-form-item">
			...
			<div class="layui-form-item">
                ...
                <div class="layui-input-inline">
                  <input type="text" name="phone" id="phone" value="<?php echo ($user_data['phone']); ?>" lay-verify="required" placeholder="请输入手机号码" autocomplete="off" class="layui-input">
                </div>
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-inline">
                  <input type="text" name="email" value="<?php echo ($user_data['email']); ?>" lay-verify="required" autocomplete="off" placeholder="请输入电话号码" class="layui-input">
                </div>
            </div>
            <div class="layui-form-item">
                ...
                <div class="layui-input-inline">
					...
					<div class="layui-input-inline">
					  <input type="text" name="address" id="address" value="<?php echo ($user_data['address']); ?>" lay-verify="" autocomplete="off" placeholder="请输入详细地址" class="layui-input">
					</div>
            </div>
		<hr/>
		<div class="layui-form-item">
	...
	layui.use(['layer', 'form' ,'jquery','laydate','upload'], function(){
	  ...
	  ,form = layui.form;
		...
		form.on("submit(submit)",function(data){
			...
			setTimeout(function(){
				$.post("<?php echo U('Rule/edit_admin');?>",{group_ids:_list,username:username,phone:phone,email:email,password:password,status:status,id:id,avatar:avatar,person:person,weixin:weixin,qq:qq,province:province,city:city,address:address},function(data){
