...
<html>
	<head>
		...
		<title><?php echo systemCache('web_info.store_name');?> - <?php echo ($title_city); echo ($title_classid?$title_classid:'建造师'); if($range == 1): ?>证书<?php else: ?>招聘<?php endif; ?>招聘网_<?php if($title_city or $title_classid): echo ($title_cid); echo ($title_classid); ?>招聘价格<?php else: ?>一级二级建造师招聘_人才证书招聘<?php endif; ?></title>
		<!-- <title><?php echo ($title_city); echo ($title_classid?$title_classid:'建造师'); if($range == 1): ?>证书<?php else: ?>招聘<?php endif; ?>招聘网_<?php if($title_city or $title_classid): echo ($title_cid); echo ($title_classid); ?>招聘价格<?php else: ?>一级二级建造师招聘_人才证书招聘<?php endif; ?> - <?php echo systemCache('web_info.store_name');?></title> -->
	...
	<body>
		...
		<div class="Nnav">
			<div class="w12">
				...
				<?php if($Other_top_ad == true): if(is_array($Other_top_ad)): foreach($Other_top_ad as $key=>$v): ?><div class="n_bann" style="background: url(<?php echo ($v['ad_code']); ?>) no-repeat center top;"></div><?php endforeach; endif; endif; ?>
		<div class="ny_cen">
			<div class="left">
				<div class="a1tit">
					<h3>招聘信息</h3>
				...
				<div class="type">
					<a href="/zp/r0t0c0.html" <?php if(($range) == "0"): ?>class="on"<?php endif; ?>>招聘信息</a>
					<a href="/zp/r1t0c0.html" <?php if(($range) == "1"): ?>class="on"<?php endif; ?>>求职信息</a>
				</div>
				<div class="news">
					<h3>新闻动态</h3>
					<ul>
					<?php if(is_array($about_news)): foreach($about_news as $key=>$v): ?><li><a href="/info/<?php echo idEncode($v['news_id']);?>.html" target="_blank"><?php echo getSubstr($v['title'],0,18);?></a></li><?php endforeach; endif; ?>
					</ul>
				</div>
			</div>
			<div class="right">
				<div class="tit">
					<h3><?php if($range == 0): ?>招聘信息<?php elseif($range == 1): ?>求职信息<?php endif; ?></h3>
					<p>当前位置：首页 > <?php if($range == 0): ?>招聘信息<?php elseif($range == 1): ?>求职信息<?php endif; ?></p>
				</div>
				...
				<div class="layui-form">
				  <table class="layui-table" lay-size="lg">
					...
					<?php if(is_array($data)): foreach($data as $key=>$v): $one_user = M('admin_users')->field('person')->where(array('id'=>$v['user_id']))->find(); ?>
				      <tr>
						<td><a href="/zp/<?php echo idEncode($v['id']);?>.html" target="_blank"><?php echo str_replace(I('get.wd'),'<font style="color:red;">'.I('get.wd').'</font>',$v['title']);?></a></td>
				        <td><?php echo ($one_user['person']); ?></td>
					    <td><?php echo get_diqu_name($v['province']);?>-<?php echo get_diqu_name($v['city']);?></td>
					    <td><?php echo word_time($v['addtime']);?></td>
				      </tr><?php endforeach; endif; ?>
				    </tbody>
				  </table>
				</div>
				
				<div class="zpage"><?php echo ($page); ?></div>
			</div>
		</div>
