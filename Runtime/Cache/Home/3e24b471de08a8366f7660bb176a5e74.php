...
<html>
	<head>
		...
		<title><?php echo systemCache('web_info.store_name');?> - <?php echo C('GET_NEWS_TYPE')[I('get.type')][0];?></title>
		<!-- <title><?php echo C('GET_NEWS_TYPE')[I('get.type')][0];?> - <?php echo systemCache('web_info.store_name');?></title> -->
	...
	<body>
		...
		<div class="ny_cen">
			<div class="left">
				...
				<div class="type">
					<?php $__FOR_START_16507__=1;$__FOR_END_16507__=C('NEWS_TYPE')[count];for($i=$__FOR_START_16507__;$i < $__FOR_END_16507__;$i+=1){ ?><a href="/news/<?php echo C('NEWS_TYPE')[$i][1];?>.html" <?php if($_GET['type']== C('NEWS_TYPE')[$i][1]): ?>class="on"<?php endif; ?>><?php echo C('NEWS_TYPE')[$i][0];?></a><?php } ?>
				</div>
				<div class="news">
					<h3>新闻动态</h3>
					<ul>
			...
			<div class="right">
				<div class="tit">
					<h3><?php echo C('GET_NEWS_TYPE')[I('get.type')][0];?></h3>
					<p>当前位置：首页 > 资讯 > <?php echo C('GET_NEWS_TYPE')[I('get.type')][0];?></p>
				</div>
				<div class="news_list">
			    	<ul>
					<?php if(is_array($data)): foreach($data as $key=>$v): $strtotime = strtotime($v['addtime']) ?>
			        	<li>
			            	<h3><a href="/info/<?php echo idEncode($v['news_id']);?>.html"><?php echo ($v['title']); ?></a></h3>
			                <p><?php echo mb_substr(clear_all(htmlspecialchars_decode($v['content'])),0,100,'utf-8');?></p>
			                <span><?php echo (date("Y-m-d",$strtotime)); ?></span>
			                <a href="/info/<?php echo idEncode($v['news_id']);?>.html" class="right">+ 查看</a></span>
