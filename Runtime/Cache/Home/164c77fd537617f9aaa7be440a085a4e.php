...
<html>
	<head>
		...
		<title><?php echo systemCache('web_info.store_name');?> - <?php echo ($one['title']); ?></title>
		<!-- <title><?php echo ($one['title']); ?> - <?php echo systemCache('web_info.store_name');?></title> -->
 ...
 if(S('about_select_type_1')){ $about_select_type_1 = S('about_select_type_1'); }else{ $about_select_type_1 = get_select(M('about'),array('type'=>1,'show'=>1),'about_id asc',10,'about_id,title,url'); S('about_select_type_1',$about_select_type_1,270); } if(S('about_select_type_3')){ $about_select_type_3 = S('about_select_type_3'); }else{ $about_select_type_3 = get_select(M('about'),array('type'=>3,'show'=>1),'about_id asc',10,'about_id,title,url'); S('about_select_type_3',$about_select_type_3,270); } if(S('news_type_1')){ $news_type_1 = S('news_type_1'); }else{ $news_type_1 = get_select(M('news'),array('type'=>1,'show'=>1),'news_id desc',3,'news_id,title'); S('news_type_1',$news_type_1,120); } if(S('Other_top_ad')){ $Other_top_ad = S('Other_top_ad'); }else{ $ad = M('ad'); $ad_field = 'ad_link,ad_code,target'; $ad_top_map['pid'] = 2; $ad_top_map['show'] = 1; $Other_top_ad = get_select($ad,$ad_top_map,'ad_id',C('AD_POSITION')[1][1],$ad_field); S('Other_top_ad',$Other_top_ad,300); } ?>
	<body>
		...
		<div class="ny_cen">
			<div class="left">
				<div class="a1tit">
					<!-- <h3><?php echo C('HOME_HELP')[$one['type']];?></h3> -->
					<h3>关于我们</h3>
					<p>专业为企业解决问题</p>
				</div>
				<div class="type">
					<?php if(is_array($select)): foreach($select as $key=>$vo): ?><a <?php if(($about_id) == $vo['url']): ?>class="on"<?php endif; ?> href="/help/<?php echo ($vo['url']); ?>.html"><?php echo ($vo['title']); ?></a><?php endforeach; endif; ?>
				</div>
				<div class="news">
					<div class="a1tit" style="color: #fff;font-size: 24px;">联系方式</div>
					<div style="background-color: #f2f2f2;">
						<p style="line-height: 30px;font-size: 14px;margin-left: 10px;">服务咨询热线：</p>
						<p style="line-height: 30px;font-size: 14px;margin-left: 10px;">QQ：</p>
						<p style="line-height: 30px;font-size: 14px;margin-left: 10px;">公司地址：</p>
					</div>
					<!-- <ul>
			...
			<div class="right">
				<div class="tit">
					...
					<p>当前位置：首页 > <?php echo C('HOME_HELP')[$one['type']];?> > <?php echo ($one['title']); ?></p>
				</div>
				<div class="cont"><?php echo ($one['content']); ?></div>
			</div>
		</div>
