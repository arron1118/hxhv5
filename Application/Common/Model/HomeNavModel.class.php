...
class HomeNavModel extends BaseModel{
	...
	public function getTreeData($type='tree',$order=''){
		...
		if($type=='tree'){
			...
			$auth=new \Think\Auth();
			foreach ($data as $k => $v) {
				if ($auth->check($v['mca'],$_SESSION['admin_user']['id'])) {
					foreach ($v['_data'] as $m => $n) {
						if(!$auth->check($n['mca'],$_SESSION['admin_user']['id'])){
	...
	//前台获取菜单
	public function getSubData($pid=0,$order='order_number'){
		$data=$this->where('pid='.$pid)->order($order)->select();
		foreach ($data as $k => $v) {
			$data[$k]['sub_nav']=$this->where('pid='.$v['id'])->order($order)->select();
		}
		return $data;
	}
